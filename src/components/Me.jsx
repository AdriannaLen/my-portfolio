const Me = () => {
  return (
    <div id="me" className="bg-whitePeach pb-[2rem] relative min-h-[100vh] flex flex-col lg:flex-row lg:justify-start items-start">
      {/* Pionowa linia */}
      <div className="hidden sm:block absolute h-full w-[0.125rem] bg-slate-600 left-[1.5rem] sm:left-[12rem] lg:left-[15%] top-0"></div>

      {/* Sekcja tekstowa */}
      <div className="mt-[8rem] sm:ml-[14rem] lg:ml-[20%] flex flex-col lg:items-start h-full w-full lg:w-auto">
        ...
        <div className="flex-1">
            <h2 className="font-poppins mt-0 w-full sm:w-[70%] md:w-full lg:w-[60%] text-text text-[0.875rem] sm:text-[1rem] md:text-[1.125rem] lg:text-[1.125rem] leading-relaxed space-y-[1rem] p-[1.5rem] rounded-lg">
              Graduated Kazimierz Wielki University and Software Development Academy as a Front-End Developer
            </h2>
        </div>

        <button type="submit" className="w-auto bg-lightCoral text-white p-4 rounded-2xl font-bold hover:bg-customBlue transition duration-300 mt-32 ">
